...
export default defineConfig({
  ...
  optimizeDeps: {
    ...
    //
    // Keep the dependency optimizer focused on application-side packages. TanStack
    // Start owns its browser hydration entry through its virtual dev entry and plugin
    // environment configuration. Do not force-include @tanstack/react-start/client or
    // @tanstack/react-start-client here: their shared client-core graph references the
    // server-only start-storage-context package, whose node:async_hooks import is
    // externalized by Vite and crashes the browser with "AsyncLocalStorage is not a
    // constructor". The Start plugin handles the correct client/server split itself.
    include: [
      'react',
      'react-dom',
      'react-dom/client',
