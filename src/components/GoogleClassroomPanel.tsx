...
import { createGoogleClassroomWork, fetchGoogleClassroomCourses, fetchGoogleClassroomWork, getGoogleIntegrationStatus, type ClassroomCourse, type ClassroomWork } from '@/lib/google-calendar-api'
...
export function GoogleClassroomPanel({ management = false }: { management?: boolean }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setConnected(Boolean(status.connected)); setEmail(status.email || ''); setRole(status.role || 'student')
      if (management) {
      ...
      if (status.connected) {
        ...
        if (selected) setWork(await fetchGoogleClassroomWork(selected))
      }
  ...
  useEffect(() => { void load() }, [])
  const selectCourse = async (id: string) => { setCourseId(id); try { setWork(await fetchGoogleClassroomWork(id)) } catch (error) { toast.error('Coursework could not load', { description: error instanceof Error ? error.message : 'Please try again.' }) } }
