...
const NAV_ITEMS: NavItemDef[] = [
  ...
  { href: '/test-mode', icon: <ClipboardCheck className="h-4 w-4" />, label: 'Test Mode' },
]
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        >
          {!collapsed && (
            <>
              <div className="flex items-center justify-center h-7 w-7 rounded-md bg-card text-primary-foreground text-xs font-bold shrink-0 overflow-hidden">
                <img src="/brand/eleviq-logo.png" alt="ELEVIQ Prep" className="h-full w-full object-contain" />
              </div>
              <span className="flex-1 font-semibold text-sm truncate">ELEVIQ Prep</span>
            </>
          )}
          <Tooltip>
          ...
          {collapsed ? (
              ...
              <TooltipTrigger asChild>
                <button type="button" className="flex items-center justify-center h-8 w-8 rounded-md hover:bg-accent transition-colors cursor-pointer">
                  <Avatar className="h-6 w-6 shrink-0">
                    <AvatarFallback className="text-[10px] bg-muted">E</AvatarFallback>
                  </Avatar>
                </button>
              </TooltipTrigger>
              <TooltipContent side="right">Student profile</TooltipContent>
            </Tooltip>
          ) : (
            <button type="button" className="flex items-center gap-2 rounded-md hover:bg-accent transition-colors cursor-pointer w-full px-2 py-1.5">
              ...
              <div className="flex-1 min-w-0 text-left">
                <p className="text-xs font-medium leading-tight truncate">Student</p>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  onClick={() => void blink.auth.logout()}
                >
