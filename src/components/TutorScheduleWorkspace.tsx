...
import { CalendarDays, CheckCircle2, Clock3, Link2, LoaderCircle, UserRound, Video, X } from 'lucide-react'
import { toast } from 'sonner'
import { Button } from '@/components/ui/button'
import { Card, CardContent } from '@/components/ui/card'
import { createGoogleCalendarEvent, getGoogleIntegrationStatus } from '@/lib/google-calendar-api'
...
export function TutorScheduleWorkspace({ onClose }: { onClose: () => void }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [people, booked, status] = await Promise.all([fetchSchedulePeople(), fetchScheduledSessions(), getGoogleIntegrationStatus()])
  ...
  // The loader intentionally updates local state after the async scheduling requests.
    ...
    try {
      ...
      if (calendarConnected && (saved.studentEmail || saved.tutorEmail)) {
        try {
          ...
          if (calendar.meetingUri) setMeetingLinks(current => ({ ...current, [saved.id]: calendar.meetingUri as string }))
          toast.success('Session booked with Calendar + Meet', { description: calendar.meetingUri ? 'The calendar invitation includes a Google Meet link.' : 'The tutoring session is now visible in the schedule.' })
        } catch (calendarError) {
