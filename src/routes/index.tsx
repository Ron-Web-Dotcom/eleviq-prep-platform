    ...
    <main className="min-h-dvh overflow-hidden bg-background">
      <nav className="relative z-20 mx-auto flex w-full max-w-7xl items-center justify-between border-b border-primary/10 px-5 py-4 lg:border-0 lg:px-10 lg:py-6">
        <Link to="/" className="group flex shrink-0 items-center" aria-label="ELEVIQ Prep home">
          <img src="/brand/eleviq-logo.png" alt="ELEVIQ Prep" className="eleviq-logo h-16 w-52 object-contain object-center transition-transform duration-200 group-hover:-translate-y-0.5 sm:h-18 sm:w-60" />
        </Link>
        ...
        <div className="hidden items-center gap-2 md:flex"><Link to="/login" search={{ next: '/app' }} className="rounded-full px-4 py-3 text-sm font-bold text-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-secondary active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Student login</Link><Link to="/login" search={{ next: '/admin' }} className="rounded-full px-4 py-3 text-sm font-bold text-muted-foreground transition-all duration-200 hover:-translate-y-0.5 hover:bg-secondary hover:text-primary active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Staff access</Link><button type="button" onClick={() => openContact()} className="group inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/20 transition-all duration-200 hover:-translate-y-1 hover:shadow-xl active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">Start preparing <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-1" /></button></div>
      ...
      <section id="pn-hesi" className="relative overflow-hidden border-y border-border bg-secondary/40 px-5 py-16 sm:px-6 lg:px-10 lg:py-24">
        <div className="pointer-events-none absolute -right-24 top-10 h-80 w-80 rounded-full bg-primary/10 blur-3xl" />
        <div className="relative mx-auto max-w-6xl">
          <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.22em] text-primary">ELEVIQ Prep</p><h2 className="mt-3 font-serif text-4xl font-bold leading-tight text-primary sm:text-5xl">PN HESI Exit Bootcamp</h2><p className="mt-4 text-base font-semibold text-muted-foreground">3-Month · 1-on-1 Intensive Exam Preparation</p><p className="mt-5 max-w-2xl leading-7 text-muted-foreground">Strengthen nursing knowledge, clinical judgment, test-taking strategies, and confidence before your HESI PN Exit Exam with personalized instruction and individualized remediation.</p></div>
          <div className="mt-8 grid gap-4 lg:grid-cols-2">{pnHesiPackages.map((pkg, index) => <article key={pkg.name} className={`relative flex flex-col rounded-[1.35rem] border p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${index === 1 ? 'border-primary bg-primary text-primary-foreground shadow-xl' : 'border-border bg-card text-card-foreground'}`}>
            {index === 1 && <span className="absolute right-5 top-5 rounded-full bg-accent px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-accent-foreground">Recommended</span>}
            <p className={`text-xs font-bold uppercase tracking-[0.18em] ${index === 1 ? 'text-accent' : 'text-primary'}`}>0{index + 1} · 3 days per week</p>
            <h3 className={`mt-4 font-serif text-3xl font-bold ${index === 1 ? 'text-primary-foreground' : 'text-card-foreground'}`}>{pkg.name}</h3>
            <p className={`mt-2 text-sm leading-6 ${index === 1 ? 'text-primary-foreground/75' : 'text-muted-foreground'}`}>{pkg.detail}</p>
            <p className={`mt-5 text-xs font-bold uppercase tracking-[0.16em] ${index === 1 ? 'text-accent' : 'text-primary'}`}>{index === 0 ? 'Includes' : 'Includes everything in Lite, plus'}</p>
            <ul className="mt-4 grid gap-x-5 gap-y-2.5 sm:grid-cols-2">{pkg.features.map(feature => <li key={feature} className="flex items-start gap-2 text-sm leading-5"><Check className={`mt-0.5 h-4 w-4 shrink-0 ${index === 1 ? 'text-accent' : 'text-primary'}`} /><span className={index === 1 ? 'text-primary-foreground/80' : 'text-muted-foreground'}>{feature}</span></li>)}</ul>
            <button type="button" onClick={() => openContact(`PN HESI ${pkg.name}`)} className={`mt-7 rounded-lg px-4 py-2.5 text-sm font-bold transition-all hover:-translate-y-0.5 active:scale-95 ${index === 1 ? 'bg-accent text-accent-foreground' : 'bg-primary text-primary-foreground'}`}>Choose {pkg.name} <ArrowRight className="ml-2 inline h-4 w-4" /></button>
          </article>)}</div>
        </div>
      </section>

      <section id="books" className="relative overflow-hidden border-y border-border bg-secondary/40 px-6 py-20 lg:px-10 lg:py-28">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[0.92fr_1.08fr]">
          <div><div className="flex items-center gap-3"><span className="h-px w-10 bg-primary" /><p className="text-xs font-bold uppercase tracking-[0.22em] text-primary">The study shop</p></div><h2 className="mt-5 max-w-xl font-serif text-4xl font-bold leading-[1.05] tracking-[-0.035em] text-primary sm:text-5xl lg:text-6xl">Tools for the hours between sessions.</h2><p className="mt-6 max-w-lg text-base leading-7 text-muted-foreground">Browse focused Phlebotomy and CNA workbooks made for clearer review, stronger recall, and less guesswork between tutoring sessions.</p><div className="mt-8 flex flex-wrap gap-3"><a href={phlebotomyAmazonUrl} target="_blank" rel="noreferrer" className="group inline-flex min-h-11 items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:-translate-y-1 hover:shadow-xl active:translate-y-0">Shop Phlebotomy <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></a><a href={cnaAmazonUrl} target="_blank" rel="noreferrer" className="group inline-flex min-h-11 items-center gap-2 rounded-xl border border-primary/20 bg-background/70 px-5 py-3 text-sm font-bold text-primary transition-all hover:-translate-y-1 hover:bg-card">Shop CNA <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></a></div><p className="mt-5 text-xs font-medium text-muted-foreground">Official ELEVIQ listings on Amazon · opens in a new tab</p></div>
          <div className="relative"><div className="grid gap-4 sm:grid-cols-2">{workbooks.map((workbook, index) => <article key={workbook.name} className={`group relative min-h-[330px] overflow-hidden rounded-[1.6rem] border p-6 shadow-lg transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${workbook.featured ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-card-foreground'}`}>
            <div className="flex items-start justify-between"><div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${workbook.featured ? 'bg-primary-foreground/15 text-primary-foreground' : 'bg-secondary text-primary'}`}><BookOpen className="h-5 w-5" /></div><span className={`font-mono text-xs font-bold ${workbook.featured ? 'text-primary-foreground/60' : 'text-muted-foreground'}`}>0{index + 1}</span></div>
            <div className="mt-14 pb-12"><p className={`text-xs font-bold uppercase tracking-[0.15em] ${workbook.featured ? 'text-accent' : 'text-primary'}`}>{workbook.eyebrow}</p><h3 className={`mt-3 font-serif text-3xl font-bold leading-tight ${workbook.featured ? 'text-primary-foreground' : 'text-card-foreground'}`}>{workbook.name}</h3><p className={`mt-4 text-sm leading-6 ${workbook.featured ? 'text-primary-foreground/75' : 'text-muted-foreground'}`}>{workbook.detail}</p></div>
            <a href={workbook.url} target="_blank" rel="noreferrer" className={`absolute bottom-6 left-6 inline-flex items-center gap-2 text-sm font-bold underline decoration-2 underline-offset-4 transition-transform group-hover:translate-x-1 ${workbook.featured ? 'text-primary-foreground decoration-accent' : 'text-primary decoration-accent'}`}>View on Amazon <ArrowRight className="h-4 w-4" /></a>
          </article>)}</div><div className="mt-4 flex items-center gap-5 rounded-[1.4rem] border border-border bg-card p-5 shadow-md"><div><p className="text-4xl font-bold tracking-[-0.05em] text-primary">24/7</p><p className="mt-1 text-sm font-bold text-card-foreground">Practice access</p></div><div className="h-10 w-px bg-border" /><p className="max-w-xs text-sm leading-6 text-muted-foreground">Keep your study rhythm moving when your schedule opens up.</p></div></div>
        </div>
      </section>
      ...
      <section id="contact" className="relative overflow-hidden border-y border-border bg-secondary/40 px-6 py-20 lg:px-10 lg:py-28">
        <div className="pointer-events-none absolute -right-24 -top-28 h-80 w-80 rounded-full bg-primary/10 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-36 left-1/3 h-80 w-80 rounded-full bg-accent/10 blur-3xl" />
        <div className="relative mx-auto grid max-w-7xl items-end gap-10 lg:grid-cols-[1fr_auto]">
          <div>
            <div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary font-mono text-[10px] font-bold text-primary-foreground">04</span><p className="text-xs font-bold uppercase tracking-[0.22em] text-primary">Ready when you are</p></div>
            <h2 className="mt-6 max-w-3xl font-serif text-5xl font-bold leading-[0.98] tracking-[-0.045em] text-primary sm:text-6xl lg:text-8xl">Make your study time <span className="italic text-accent">count.</span></h2>
            <p className="mt-6 max-w-xl text-base leading-7 text-muted-foreground">Start with a conversation, choose your path, and build a study rhythm that feels possible.</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row lg:flex-col lg:pb-1"><button type="button" onClick={() => openContact()} className="group inline-flex min-h-12 items-center justify-center gap-3 rounded-xl border border-primary/20 bg-background/70 px-6 py-3 text-sm font-bold text-primary transition-all hover:-translate-y-1 hover:bg-card">Contact us <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></button><button type="button" onClick={() => scrollToSection('pn-hesi')} className="group inline-flex min-h-12 items-center justify-center gap-3 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:-translate-y-1 hover:shadow-xl">Find your bootcamp <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></button></div>
        </div>
      </section>
