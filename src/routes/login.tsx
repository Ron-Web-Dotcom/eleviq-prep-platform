...
function AuthBrandPanel() {
  return <section className="relative hidden min-h-[620px] overflow-hidden rounded-[2rem] border border-primary-foreground/15 bg-primary p-8 shadow-2xl shadow-primary/25 lg:flex lg:flex-col lg:justify-between xl:p-10"><div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-accent/20 blur-3xl" /><div className="absolute -bottom-28 -left-20 h-72 w-72 rounded-full bg-primary-foreground/10 blur-3xl" /><div className="relative"><div className="flex items-center gap-3"><div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-2xl bg-secondary p-1.5 shadow-lg"><img src="/brand/eleviq-logo.png" alt="ELEVIQ Prep" className="h-full w-full object-contain" /></div><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.24em] text-accent">ELEVIQ / PREP</p><p className="mt-1 text-sm font-semibold text-primary-foreground">Secure learning workspace</p></div></div><p className="mt-16 max-w-md font-mono text-xs uppercase tracking-[0.18em] text-primary-foreground/60">One secure entry. Every ELEVIQ workspace.</p><h1 className="mt-4 max-w-xl font-serif text-5xl leading-[1.02] tracking-tight text-primary-foreground xl:text-6xl">Prepare with purpose.</h1><p className="mt-6 max-w-md text-sm leading-7 text-primary-foreground/75">Sign in once and ELEVIQ will take you to the workspace your account is authorized to use — student learning, tutoring, or platform operations.</p></div><div className="relative grid gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-primary-foreground/15 bg-primary-foreground/10 p-4"><p className="text-2xl font-semibold text-accent">01</p><p className="mt-2 text-xs leading-5 text-primary-foreground/70">Student learning</p></div><div className="rounded-2xl border border-primary-foreground/15 bg-primary-foreground/10 p-4"><p className="text-2xl font-semibold text-accent">02</p><p className="mt-2 text-xs leading-5 text-primary-foreground/70">Tutor guidance</p></div><div className="rounded-2xl border border-primary-foreground/15 bg-primary-foreground/10 p-4"><p className="text-2xl font-semibold text-accent">03</p><p className="mt-2 text-xs leading-5 text-primary-foreground/70">Admin clarity</p></div></div></section>
}
...
function Login() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (normalized.split('@')[1] !== 'eleviqprep.com') return setError('ELEVIQ accounts must use an @eleviqprep.com email address.')
  ...
  const title = mode === 'forgot' ? 'Reset your password' : access ? 'Request secure access' : 'Prepare with purpose.'
  const subtitle = mode === 'forgot' ? 'We’ll help you get back into your workspace.' : access ? 'Use your ELEVIQ email to receive a secure sign-in link.' : 'Sign in to continue'
  const entryLabel = adminIntent ? 'Staff and administrator entry' : 'Student entry'
  const entryDescription = adminIntent ? 'Use your ELEVIQ staff account. Your assigned role determines whether the tutor or admin workspace opens.' : 'Sign in with your ELEVIQ account, then connect your personal Google account inside the student portal.'
  const continueWithGoogle = async () => {
    setError('')
    setBusy(true)
    try {
      // Google identity and Google Workspace permissions are separate concerns:
      // first authenticate the ELEVIQ session, then request the personal Google
      // account connection through the backend OAuth flow after the callback.
      await blink.auth.signInWithGoogle()
      await startGoogleIntegration(nextPath)
    } catch (cause) {
      setBusy(false)
      setError(cause instanceof Error ? cause.message : 'Google sign-in could not start. Please try again.')
    }
  }
  return <main className="relative min-h-dvh overflow-hidden bg-background px-4 py-6 text-foreground sm:px-6 lg:bg-primary"><div className="pointer-events-none absolute -left-24 top-10 h-64 w-64 rounded-full bg-accent/15 blur-3xl lg:hidden" /><div className="relative mx-auto grid min-h-[calc(100dvh-3rem)] max-w-6xl items-center gap-8 lg:grid-cols-[1fr_380px] xl:gap-12"><AuthBrandPanel /><section className="w-full animate-fade-in rounded-[2rem] border border-border bg-card p-6 text-card-foreground shadow-lg sm:p-8"><div className="mb-6"><Link to="/" aria-label="Return to ELEVIQ home page" className="group inline-flex items-center gap-2 rounded-md px-1.5 py-1 text-xs font-semibold text-muted-foreground transition-colors hover:bg-secondary hover:text-primary"><ArrowLeft className="h-4 w-4 transition-transform duration-200 group-hover:-translate-x-0.5" /> Back to home</Link></div><div className="mb-8 flex items-center gap-3 lg:hidden"><div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl bg-secondary p-1.5"><img src="/brand/eleviq-logo.png" alt="ELEVIQ Prep" className="h-full w-full object-contain" /></div><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-accent">ELEVIQ / PREP</p><p className="mt-1 text-xs text-muted-foreground">Secure workspace access</p></div></div><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-accent">Secure entry</p><h2 className="mt-3 font-serif text-3xl font-bold tracking-tight text-primary">{title}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{subtitle}</p><div className="mt-4 rounded-xl border border-primary/15 bg-secondary/45 p-3"><p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">{entryLabel}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{entryDescription}</p></div></div><form onSubmit={submit} className="mt-8 space-y-4">{mode === 'signin' && !emailConfirmed && !adminIntent && <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground"><span className="h-px flex-1 bg-border" /><span>Account email</span><span className="h-px flex-1 bg-border" /></div>}<div><Label htmlFor="email" className="mb-2 block text-xs font-semibold text-primary">ELEVIQ email address</Label><div className="relative"><Mail className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input id="email" type="email" value={email} onChange={e => { setEmail(e.target.value); setEmailConfirmed(false); setEmailCheck('') }} className="h-11 rounded-xl border-input bg-background pl-10 text-sm text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring" /></div>{adminIntent && <p className="mt-2 text-xs leading-5 text-muted-foreground">Your account role determines which ELEVIQ workspace opens after sign-in.</p>}{emailCheck && <p className="mt-2 text-xs leading-5 text-muted-foreground" role="status">{emailCheck}</p>}</div>{mode === 'signin' && (emailConfirmed || adminIntent) && <div><div className="flex items-center justify-between"><Label htmlFor="password" className="text-xs font-semibold text-primary">Password</Label>{!adminIntent && <button type="button" onClick={() => setMode('forgot')} className="text-xs font-semibold text-primary underline-offset-4 hover:text-accent hover:underline">Forgot password?</button>}</div><div className="relative mt-2"><LockKeyhole className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} className="h-11 rounded-xl border-input bg-background pl-10 text-sm text-foreground focus-visible:ring-2 focus-visible:ring-ring" required /></div>{!adminIntent && <button type="button" onClick={() => { setEmailConfirmed(false); setPassword('') }} className="mt-2 text-xs text-muted-foreground underline-offset-4 hover:text-primary hover:underline">Use a different email</button>}</div>}{mode === 'forgot' && <p className="rounded-xl bg-secondary p-3 text-xs leading-5 text-muted-foreground">Enter the verified ELEVIQ email already in our system. We’ll email a secure link so you can choose a new password.</p>}{mode === 'forgot' && <button type="button" onClick={() => void resendVerificationEmail(email.trim().toLowerCase()).then(() => toast.success('Verification email requested', { description: 'If that address has an account, a verification link is on its way.' })).catch(cause => setError(cause instanceof Error ? cause.message : 'We could not resend the verification email.'))} className="text-left text-xs font-semibold text-primary underline-offset-4 hover:text-accent hover:underline">Need to verify this email? Resend verification</button>}{error && <p role="alert" className="rounded-xl border border-destructive/25 bg-destructive/10 p-3 text-xs leading-5 text-destructive">{error}</p>}{studentLockoutActive && <p className="rounded-xl border border-accent/30 bg-accent/10 p-3 text-xs leading-5 text-primary"><strong>Temporary password required.</strong> Check your email for the one-time password sent by an ELEVIQ administrator, then enter it above. You will be required to choose a new password.</p>}<Button disabled={busy} className="h-11 w-full rounded-xl bg-primary text-sm font-bold text-primary-foreground shadow-md shadow-primary/15 transition-transform hover:-translate-y-0.5 hover:bg-primary/90 active:translate-y-0">{busy ? 'Please wait…' : mode === 'forgot' ? 'Send reset link' : access ? 'Email secure access link' : studentLockoutActive ? 'Use temporary password' : emailConfirmed || adminIntent ? 'Sign in securely' : 'Continue with email'} {!busy && <ArrowRight className="h-4 w-4" />}</Button></form>{mode === 'signin' && !adminIntent && <><div className="my-5 flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground"><span className="h-px flex-1 bg-border" /><span>Or</span><span className="h-px flex-1 bg-border" /></div><button type="button" onClick={() => void continueWithGoogle()} disabled={busy} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-input bg-background text-sm font-bold text-foreground transition-colors hover:bg-secondary disabled:cursor-wait disabled:opacity-60"><Globe2 className="h-4 w-4" /> Continue with Google</button><p className="mt-2 text-center text-[10px] leading-4 text-muted-foreground">Use your personal Google account. You can connect Classroom after entering ELEVIQ.</p></>}{<div className="mt-7 border-t border-border pt-5 text-center text-xs text-muted-foreground">{mode === 'signin' ? <>Need an account? <button type="button" onClick={() => { setMode('access'); setEmailConfirmed(false) }} className="font-semibold text-primary underline-offset-4 hover:text-accent hover:underline">Request access</button></> : <button type="button" onClick={() => { setMode('signin'); setEmailConfirmed(false) }} className="font-semibold text-primary underline-offset-4 hover:text-accent hover:underline">Back to sign in</button>}</div>}{authNotice && <p className="mt-4 rounded-xl bg-secondary p-3 text-center text-xs leading-5 text-primary" role="status">{authNotice}</p>}<p className="mt-5 text-center text-[10px] leading-4 text-muted-foreground">By continuing, you agree to our <Link to="/terms" className="font-semibold text-primary underline">Terms of Service</Link> and <Link to="/privacy" className="font-semibold text-primary underline">Privacy Policy</Link></p></section></div></main>
}
