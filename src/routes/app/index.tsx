...
import { startGoogleIntegration, getGoogleIntegrationStatus } from '@/lib/google-calendar-api'
import { ArrowRight, BrainCircuit, CalendarDays, Clock3, Zap, Target, CheckCircle2, BookOpen, Video } from 'lucide-react'
...
function GoogleWorkspaceCard() {
  ...
  useEffect(() => {
    void getGoogleIntegrationStatus().then(status => { setConnected(Boolean(status.connected)); setEmail(status.email || '') }).catch(() => setConnected(false))
  }, [])

  const connect = async () => {
    setWorking(true)
    try { await startGoogleIntegration() } catch (error) { toast.error('Google connection could not start', { description: error instanceof Error ? error.message : 'Please try again.' }); setWorking(false) }
  }

  return <Card className="border-primary/15 bg-card/80"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-accent">Your Google workspace</p><h2 className="mt-1 font-serif text-2xl text-primary">Classroom · Calendar · Meet</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Sign in to ELEVIQ, then connect your own Google account here. Google controls which Classroom classes, Calendar events, and Meet actions you are authorized to use.</p></div><CalendarDays className="h-5 w-5 text-accent" /></div><div className="mt-4 flex flex-col gap-3 rounded-2xl bg-secondary/45 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-2 text-xs font-semibold text-primary"><span className="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1.5"><BookOpen className="h-3.5 w-3.5 text-accent" />Classroom</span><span className="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1.5"><CalendarDays className="h-3.5 w-3.5 text-accent" />Calendar</span><span className="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1.5"><Video className="h-3.5 w-3.5 text-accent" />Meet</span></div>{connected ? <p className="text-xs font-semibold text-chart-3">Connected{email ? ` · ${email}` : ''}</p> : <button type="button" disabled={working} onClick={() => void connect()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:opacity-60">{working ? 'Connecting…' : 'Connect Google account'}</button>}</div></CardContent></Card>
}

function DashboardHome() {
  return (
    <div className="relative space-y-8 overflow-hidden"><GoogleWorkspaceCard /><section className="rounded-2xl border border-primary/15 bg-card p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-accent">New study experience</p><h2 className="mt-1 font-serif text-2xl text-primary">ELEVIQ Test Mode</h2><p className="mt-1 text-sm text-muted-foreground">Practice pacing, case studies, multi-select, and clinical-judgment interactions.</p></div><a href="/app/test-mode" className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground transition-transform hover:-translate-y-0.5 active:translate-y-0">Open Test Mode <ArrowRight className="h-4 w-4" /></a></div></section>
      ...
      <div className="grid gap-6 lg:grid-cols-2"><StudentScheduleCard /><GoogleCalendarEvents compact /></div>
