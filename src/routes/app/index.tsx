...
function GoogleWorkspaceCard() {
  ...
  return <Card className="border-primary/15 bg-card/80"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-accent">Your Google workspace</p><h2 className="mt-1 font-serif text-2xl text-primary">Classroom · Calendar · Meet</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Your ELEVIQ administrator connects the teacher Classroom once. Students then see only the Google Classroom classes where their ELEVIQ email is enrolled.</p></div><CalendarDays className="h-5 w-5 text-accent" /></div><div className="mt-4 flex flex-col gap-3 rounded-2xl bg-secondary/45 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-2 text-xs font-semibold text-primary"><span className="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1.5"><BookOpen className="h-3.5 w-3.5 text-accent" />Classroom</span><span className="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1.5"><CalendarDays className="h-3.5 w-3.5 text-accent" />Calendar</span><span className="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1.5"><Video className="h-3.5 w-3.5 text-accent" />Meet</span></div>{connected ? <p className="text-xs font-semibold text-chart-3">Google connected{email ? ` · ${email}` : ''}</p> : <button type="button" disabled={working} onClick={() => void connect()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground transition-transform hover:-translate-y-0.5 disabled:opacity-60">{working ? 'Connecting…' : 'Connect personal Google'}</button>}</div></CardContent></Card>
}
